...
if (( WITH_DATA )); then
  # The seed's password (test1234) is public in the repo, so the
  # admin account gets a password that exists only on this server.
  read -rsp "New password for $ADMIN_EMAIL (min. 10 characters): " ADMIN_PW; echo
  (( ${#ADMIN_PW} >= 10 )) || { echo "Too short. Aborted, nothing changed."; exit 1; }
fi
